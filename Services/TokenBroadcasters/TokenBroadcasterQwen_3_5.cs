...
public class TokenBroadcasterQwen_3_5 : TokenBroadcasterBase
{
    private static readonly Regex ToolCallBlockRegex = new(
        @"<tool_call>\s*(?<inner>.*?)\s*</tool_call>",
    ...
    public TokenBroadcasterQwen_3_5(
    ...
    public override List<(string json, string functionName)> ParseInputForJson(string input)
    {
        ...
        foreach (Match block in ToolCallBlockRegex.Matches(input))
        {
