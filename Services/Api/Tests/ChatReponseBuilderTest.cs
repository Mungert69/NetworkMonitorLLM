...
public class ChatResponseBuilderTests
{
    ...
    [Fact]
    public void BuildResponse_HydratesEmptyToolCallArgumentsFromXml_WhenXmlEnabled()
    {
        // Arrange
        const string xmlContent = "<function_call name=\"call_cmd_processor_expert\"><parameters><message>hello</message></parameters></function_call>";
        var hfResponse = new HuggingFaceChatResponse
        {
            Choices = new List<HuggingFaceChoice>
            {
        ...
        _mockTokenBroadcaster
            ...
            .Returns(new List<(string json, string functionName)>
            {
                ("{\"message\":\"hello\",\"args_escaped\":false}", "call_cmd_processor_expert")
            });
        ...
        Assert.NotNull(toolCalls);
        Assert.Single(toolCalls!);
        Assert.Equal("{\"message\":\"hello\",\"args_escaped\":false}", toolCalls![0]!.FunctionCall!.Arguments);
        Assert.Equal(string.Empty, result.Choices[0].Message.Content);
    }

    [Fact]
    public void BuildResponse_DoesNotHydrateXml_WhenXmlDisabled()
    {
        // Arrange
        const string xmlContent = "<function_call name=\"call_cmd_processor_expert\"><parameters><message>hello</message></parameters></function_call>";
        var hfResponse = new HuggingFaceChatResponse
        ...
        Assert.NotNull(toolCalls);
        Assert.Single(toolCalls!);
        Assert.Equal("{}", toolCalls![0]!.FunctionCall!.Arguments);
        Assert.Equal(xmlContent, result.Choices[0].Message.Content);
    }
